...
async function prepareWorkLists(fileList) {
  ...
  try {
    const prepared = await Promise.all(files.map(async (file,index) => {
      const raw = await readWiFile(file); const plannerOnly = isPlannerPerformanceOnly(raw.rows); const wiRecords = normalizeWiRows(raw.rows); const moves = wiRecords.filter(r => r.unit && ["LOAD","DSCH"].includes(r.kind));
      ...
      return { id:`vessel-${index+1}`, fileName:file.name, fileSize:file.size, sheetName:raw.sheetName || "", wiRecords, name:identity.name, visit:identity.visit, multipleVisits:identity.multipleVisits, plannerOnly, totalMoves:moves.length, loads:moves.filter(r=>r.kind==="LOAD").length, discharges:moves.filter(r=>r.kind==="DSCH").length, shortSteaming:plannerOnly ? false : prior.get(file.name) || false };
    }));
    const plannerOnlyCount = prepared.filter(vessel => vessel.plannerOnly).length;
    state.preparedVessels = prepared; el.wiFileName.textContent = `${prepared.length} Work List${prepared.length===1?"":"s"} selected${plannerOnlyCount?` · ${plannerOnlyCount} planner only`:""}`; el.wiState.textContent = `${prepared.length} ready`; renderVesselSetup(); showToast(`${prepared.length} vessel${prepared.length===1?"":"s"} detected${plannerOnlyCount?` · ${plannerOnlyCount} planner performance only`:""}.`);
  } catch (error) {
...
async function runPlanningAnalysis() {
  if (el.runPlanningButton.disabled) return; state.loadingPlanning = true; updateRunStates(); const allPlannerOnly=state.preparedVessels.every(vessel=>vessel.plannerOnly); el.planningStatus.textContent = allPlannerOnly ? "Calculating planner performance…" : "Calculating planner, NVV and POW-based rehandle controls…";
  try {
    await nextFrame();
    ...
    state.currentPlanning = analysis; await saveHistory(compactHistoryRecord(analysis)); await renderHistory(); syncGlobalFilterDefaults(analysis.terminal); renderAll(); activateTab(allPlannerOnly?"planner":"overview"); el.planningStatus.textContent = `${formatNumber(analysis.planner.totalMoves)} moves processed across ${analysis.vessels.length} vessels${allPlannerOnly?" · planner performance only":""}.`; showToast(allPlannerOnly?"Planner performance analyzed and saved.":"Planning batch analyzed and saved.");
...
function renderOverview() {
  ...
  const vessels=scopedVessels(), moves=scopedMoves(), planners=new Set(moves.map(m=>m.planner||"Unassigned")); const ss=vessels.filter(v=>v.shortSteaming&&!v.plannerOnly).length, plannerOnly=vessels.filter(v=>v.plannerOnly).length, eligible=vessels.filter(v=>!v.shortSteaming&&!v.plannerOnly).length;
...
function renderPlanner(){const ready=currentPlanningMatchesTerminal();el.plannerEmpty.hidden=ready;el.plannerContent.hidden=!ready;if(!ready)return;const moves=scopedMoves();const rows=summarizeMovesByPlanner(moves);const vessels=scopedVessels();const plannerOnly=vessels.filter(v=>v.plannerOnly).length;el.plannerMeta.textContent=`${filterSummary()} · ${vessels.length} vessel${vessels.length===1?"":"s"}${plannerOnly?` · ${plannerOnly} planner performance only`:""}`;el.plannerKpis.innerHTML=renderKpis([{label:"Moves",value:moves.length,note:"Filtered scope",tone:"neutral"},{label:"Load",value:moves.filter(m=>m.kind==="LOAD").length,note:"Moves",tone:"teal"},{label:"Discharge",value:moves.filter(m=>m.kind==="DSCH").length,note:"Moves",tone:"blue"},{label:"Planners",value:rows.length,note:"Filtered scope",tone:"neutral"},{label:"Planner-only vessels",value:plannerOnly,note:"NVV / rehandles excluded",tone:plannerOnly?"blue":"neutral"},{label:"SS vessels",value:vessels.filter(v=>v.shortSteaming&&!v.plannerOnly).length,note:"Manual selection",tone:vessels.some(v=>v.shortSteaming&&!v.plannerOnly)?"warning":"neutral"}]);renderBarList(el.plannerMovesChart,rows.map(r=>({label:r.planner,value:r.totalMoves})),14);renderBarList(el.plannerMoveMix,[{label:"Load",value:moves.filter(m=>m.kind==="LOAD").length},{label:"Discharge",value:moves.filter(m=>m.kind==="DSCH").length}]);renderBarList(el.plannerSizeMix,sizeMix(moves));el.plannerTableBody.innerHTML=rows.map(r=>`<tr><td><strong>${escapeHtml(r.planner)}</strong></td><td>${r.totalMoves}</td><td>${formatPercent(moves.length?r.totalMoves/moves.length:0)}</td><td>${r.loads}</td><td>${r.discharges}</td><td>${r.full}</td><td>${r.empty}</td><td>${r.size20}</td><td>${r.size40}</td><td>${r.size45}</td><td>${r.vesselCount}</td></tr>`).join("")||emptyRow(11,"No planner rows in this filter.");}
...
function scopedRehandleRows(){if(!currentPlanningMatchesTerminal())return[];const ids=new Set(scopedVessels().filter(v=>!v.plannerOnly).map(v=>v.id));return (state.currentPlanning.rehandles?.rows||[]).filter(r=>ids.has(r.vesselId)&&(state.globalFilters.planner==="all"||(r.targetPlanner||"Unassigned")===state.globalFilters.planner));}
function scopedCrossPowRows(){if(!currentPlanningMatchesTerminal())return[];const ids=new Set(scopedVessels().filter(v=>!v.plannerOnly).map(v=>v.id));return (state.currentPlanning.rehandles?.crossPowRows||[]).filter(r=>ids.has(r.vesselId)&&(state.globalFilters.planner==="all"||(r.targetPlanner||"Unassigned")===state.globalFilters.planner));}
function renderRehandles(){const planningReady=currentPlanningMatchesTerminal();const ready=planningReady&&scopedVessels().some(v=>!v.plannerOnly);el.rehandleEmpty.hidden=ready;el.rehandleContent.hidden=!ready;if(!ready){el.rehandleEmpty.querySelector("h2").textContent=planningReady?"Planner performance-only source":"No planning batch analyzed";el.rehandleEmpty.querySelector("p").textContent=planningReady?"This export is excluded from all rehandle checks.":"Potential retrieval rehandles are calculated within POW sequence. Cross-POW stack demand is reported separately.";el.rehandleTabCount.textContent="—";return;}const rows=scopedRehandleRows(),cross=scopedCrossPowRows();const probable=rows.filter(r=>r.confidence==="probable").length,possible=rows.length-probable;el.rehandleMeta.textContent=`${filterSummary()} · same-POW retrieval sequence only`;el.rehandleKpis.innerHTML=renderKpis([{label:"Potential",value:rows.length,note:"Same POW",tone:rows.length?"warning":"success"},{label:"Affected targets",value:new Set(rows.map(r=>r.targetUnit)).size,note:"Unique loads",tone:"neutral"},{label:"Probable",value:probable,note:"Stronger evidence",tone:probable?"danger":"success"},{label:"Possible",value:possible,note:"Review constraints",tone:possible?"warning":"success"},{label:"Cross-POW demand",value:cross.length,note:"Not counted as rehandle",tone:cross.length?"blue":"neutral"}]);renderBarList(el.rehandleConfidenceChart,[{label:"Probable",value:probable},{label:"Possible",value:possible}]);el.crossPowSummary.innerHTML=metricStack([{label:"Interactions",value:formatNumber(cross.length)},{label:"Stacks",value:formatNumber(new Set(cross.map(r=>r.stack)).size)},{label:"Targets",value:formatNumber(new Set(cross.map(r=>r.targetUnit)).size)}]);renderRehandleTable();el.rehandleTabCount.textContent=formatNumber(rows.length);}
